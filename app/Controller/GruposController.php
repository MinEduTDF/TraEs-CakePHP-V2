...
class GruposController extends AppController {
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Grupo->save($this->data)) {
				$this->Session->setFlash(__('El grupo ha sido grabado.'));
				$this->redirect(array('action' => 'index'));
			} else {
	...
	function edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(__('Grupo no valido.'));
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->data)) {
			if ($this->Grupo->save($this->data)) {
				...
				$this->Session->setFlash(__('El grupo no ha sido grabado. Favor, intente nuevamente.'));
			}
		}
		if (empty($this->data)) {
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Id no valida para grupo.'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Grupo->delete($id)) {
			$this->Session->setFlash(__('Grupo borrado.'));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Grupo no fue borrado.'));
		$this->redirect(array('action' => 'index'));
	}
}
?>