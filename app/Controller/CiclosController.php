...
class CiclosController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Ciclo no valido'));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Ciclo->save($this->data)) {
				$this->Session->setFlash(__('El ciclo ha sido grabado.'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('El ciclo no fue grabado. Favor, intente nuevamente.'));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Id no valido para ciclo.'));
			$this->redirect(array('action'=>'index'));
		}
		...
		$this->Session->setFlash(__('Ciclo no fue borrado.'));
		$this->redirect(array('action' => 'index'));
	}
}
?>