...
class InventariosController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Inventario no valido.', true));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Inventario->save($this->data)) {
				$this->Session->setFlash(__('El inventario ha sido grabado.', true));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('El inventario no ha sido grabado. Favor, intente nuevamente.', true));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Id no valido para inventario.', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Inventario->delete($id)) {
			$this->Session->setFlash(__('Inventario borrado.', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Inventario no fue borrado.', true));
		$this->redirect(array('action' => 'index'));
	}
}
?>