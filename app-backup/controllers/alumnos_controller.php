...
class AlumnosController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Alumno no valido', true));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Alumno->save($this->data)) {
				$this->Session->setFlash(__('El alumno ha sido grabado', true));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('El alumno no ha sido grabado. Favor, intentelo nuevamente.', true));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Id no valido para alumno', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Alumno->delete($id)) {
			$this->Session->setFlash(__('Alumno borrado', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Alumno no fue borrado', true));
		$this->redirect(array('action' => 'index'));
	}
}
?>