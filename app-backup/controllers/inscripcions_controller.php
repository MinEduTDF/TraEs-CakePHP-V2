...
class InscripcionsController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Inscripcion no valida.', true));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Inscripcion->save($this->data)) {
				$this->Session->setFlash(__('La inscripcion ha sido grabada.', true));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('La inscripcion no ha sido grabada. Favor, intente nuevamente.', true));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Id no valida para inscripcion.', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Inscripcion->delete($id)) {
			$this->Session->setFlash(__('Inscripcion borrada.', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Inscripcion no fue borrada.', true));
		$this->redirect(array('action' => 'index'));
	}
}
?>